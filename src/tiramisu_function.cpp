 ...
 */
                                        ...
                                        std::unordered_set<computation *> &visited)
    {
        ...
        {
            const std::string level = ((edge.second == computation::root_dimension) ? "root" : std::to_string(edge.second));
  ...
  */
    bool function::should_parallelize(const std::string &comp, int lev) const
    {
        ...
        std::string str = "Dimension " + std::to_string(lev) +
                          (found ? " should" : " should not") + " be mapped to CPU thread.";
...
*/
    int function::get_unrolling_factor(const std::string &comp, int lev) const
    {
        ...
        std::string str = "Dimension " + std::to_string(lev) +
                          (found ? " should" : " should not") + " be unrolled with a factor of " +
...
*/
    int function::get_vector_length(const std::string &comp, int lev) const
    {
        ...
        std::string str = "Dimension " + std::to_string(lev) +
                          (found ? " should" : " should not") + " be vectorized with a vector length of " +
...
*/
    bool function::should_vectorize(const std::string &comp, int lev) const
    {
        ...
        std::string str = "Dimension " + std::to_string(lev) +
                          (found ? " should" : " should not") + " be vectorized.";
        DEBUG(10, tiramisu::str_dump(str));
    ...
    bool function::should_distribute(const std::string &comp, int lev) const
    {
        ...
        std::string str = "Dimension " + std::to_string(lev) +
                          (found ? " should" : " should not") + " be distributed.";
        DEBUG(10, tiramisu::str_dump(str));
 ...
 */
  ...
  */
    ...
    /**
     * Generate an isl AST for the function after transformation.
     */
    void function::gen_isl_ast_after_trans()
    {
        ...
        //Differnce between normal and after transformation generation
    ...
    bool tiramisu::function::should_map_to_gpu_block(const std::string &comp, int lev0) const
    {
        ...
        std::string str = "Dimension " + std::to_string(lev0) +
                          (found ? " should" : " should not") + " be mapped to GPU block.";
        DEBUG(10, tiramisu::str_dump(str));
    ...
    bool tiramisu::function::should_map_to_gpu_thread(const std::string &comp, int lev0) const
    {
        ...
        std::string str = "Dimension " + std::to_string(lev0) +
                          (found ? " should" : " should not") + " be mapped to GPU thread.";
        DEBUG(10, tiramisu::str_dump(str));
    ...
    void tiramisu::function::set_arguments(const std::vector<tiramisu::buffer *> &buffer_vec)
    {
        ...
        for (auto &buffer : buffer_vec)
        {
            ...
            if (buffer->get_argument_type() == a_temporary)
            {
                ...
                {
                    if (comp->get_buffer() == buffer && comp->should_schedule_this_computation())
                    {
                        is_input = false;
                        break;
                    }
                }
                DEBUG(3, tiramisu::str_dump("Setting type of buffer " + buffer->get_name() + " to " + (is_input ? "input" : "output")));
    ...
    void tiramisu::function::gen_flexnlp_autocopy()
    {
        ...
        for (auto comp : this->get_computations())
        {
            ...
            {                                            // If the computation is a function call
                ...
                if (e.get_name() == "tiramisu_flexnlp_lstm_cell")
                { // If this is an lstm call
                    std::vector<tiramisu::var> v = comp->get_iteration_variables();

                    int number_of_loops = v.size();
                    if (number_of_loops == 0)
                    { // Only one cell execution, just copy the weights and the input beforehand
                    }
                    else if (number_of_loops == 1)
                    { // Consider the loop as a sequence loop
                    }
                    else
                    { // case >=2 (take two innermost loops and consider them as (LAYERS LOOP, SEQUENCE LOOP))
                ...
                else if (e.get_name() == "tiramisu_flexnlp_gru")
                { // GRU case
    ...
    void tiramisu::function::gen_halide_bug_workaround_computations()
    {
        ...
        for (auto &buffer : this->get_arguments())
        {                                              // Go through the function argument buffers (those passed in the codegen function in the Tiramisu generator)
            ...
            if (buffer->get_argument_type() == a_output && ( // Is an input buffer
                                                               buffer->get_elements_type() == p_int8 ||
                                                               buffer->get_elements_type() == p_float32))
            { // Only float32 type is supported TODO:FLEXNLP Support other types by making a buffer for each output (make a dedicated b_dummy_input_accesses buffer and a dedicated tmp_dummy_comp input for each output buffer)
                ...
                (*copy_comp).then(*copy_back_comp, computation::root).then(*first_cpt, computation::root);
...
bool tiramisu::function::loop_unrolling_is_legal(tiramisu::var i , std::vector<tiramisu::computation *> fused_computations)
{
    ...
    */
    ...
    bool tiramisu::function::loop_vectorization_is_legal(tiramisu::var i, std::vector<tiramisu::computation *> fused_computations)
    {
        ...
        bool result = this->loop_unrolling_is_legal(i, fused_computations) && this->loop_parallelization_is_legal(i, fused_computations);
