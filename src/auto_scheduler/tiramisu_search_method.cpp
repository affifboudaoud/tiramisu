...
std::vector<syntax_tree*> beam_search::explore_node_classic(syntax_tree& ast, std::vector<std::string> *schedules_annotations, candidate_trace *parent_trace, float schedule_timeout)
{
    ...
    while (iterator != children.end())
    {
        ...
        if (!child->ast_is_legal()) {
            ...
            child->evaluation = min_eval(measurements);
            std::cout<<"before child path"<<std::endl;
            parent_trace->add_child_path(child, schedules_annotations->size());
...
void beam_search::explore_parallelisation(syntax_tree& ast, std::vector<std::string> *schedules_annotations, candidate_trace *parent_trace, float schedule_timeout)
{
    ...
    optimization_type optim_type = optimization_type::PARALLELIZE;
...
void beam_search::search_save_matrix(std::vector<syntax_tree*> asts, std::vector<std::string> *schedules_annotations, candidate_trace *parent_trace, float schedule_timeout)
{
    if(asts.size()<1) return;
    std::vector<syntax_tree*> to_be_explored_global;
    std::cout<<"starting search save matrix"<<std::endl;
    for (syntax_tree* ast: asts){
    ...
    std::vector<syntax_tree*> returened_asts;
    std::cout<<ast->search_depth<<MAX_MAT_DEPTH<<ast->search_depth<<ast->nb_explored_matrices<<std::endl;
    std::cout<<"get_exploration_trace_json"<<parent_trace->get_exploration_trace_json()<<std::endl;
    if (ast->search_depth<MAX_MAT_DEPTH && ast->search_depth<=ast->nb_explored_matrices ){
        
        if (ast->search_depth == 0){std::cout<<"search_depth == 0"<<std::endl;;returened_asts =  explore_node(*ast, schedules_annotations, parent_trace, schedule_timeout);}else{
        returened_asts =  explore_node(*ast, schedules_annotations, parent_trace, schedule_timeout);}
    }else{
        std::cout<<"before update node classic"<<std::endl;
        
        std::cout<<"exploration root"<<&parent_trace<<std::endl;
        //std::cout<<"exploration param"<<parent_trace->child_mappings[ast]->get_candidate_id()<<std::endl;
        ...
        std::cout<<"search depth"<<ast->search_depth<<std::endl;
        std::cout<<"get schedule str"<<ast->get_schedule_str()<<std::endl;
        std::cout<<"ast pointer"<<ast<<std::endl;
        returened_asts =  explore_node_classic(*ast, schedules_annotations, parent_trace, schedule_timeout);
        std::cout<<"after explore node classic"<<std::endl;  
    }
    std::cout<<"after update nodes "<<std::endl;
    to_be_explored_global.insert( to_be_explored_global.end(), returened_asts.begin(), returened_asts.end() );
    }
    std::sort(to_be_explored_global.begin(), to_be_explored_global.end(), [](syntax_tree *a, syntax_tree *b) {
       return a->evaluation < b->evaluation;
    });
    // shuffle the children so that they are selected a random
    //std::shuffle(std::begin(to_be_explored), std::end(to_be_explored), rand_generator);
    // keep the top 'beam_size' children and delete the rest
    for (int i = beam_size; i < to_be_explored_global.size(); ++i)
       delete to_be_explored_global[i];
    
    for(auto child: to_be_explored_global){
        child->search_depth = child->search_depth + 1;
    }
    to_be_explored_global.resize(std::min(beam_size, (int)to_be_explored_global.size()));

    std::cout<<"to_be_explored_global size"<<to_be_explored_global.size()<<std::endl;
    search_save_matrix(to_be_explored_global,schedules_annotations,parent_trace,schedule_timeout);
}
std::vector<syntax_tree*> beam_search::explore_node(syntax_tree& ast, std::vector<std::string> *schedules_annotations, candidate_trace *parent_trace, float schedule_timeout)
{
    std::cout<<"starting uxplore node"<<std::endl;
    ...
    std::cout<<"after get matrices"<<std::endl;
    ...
    // stop if no more optimizations can be applied
    if (children.size() == 0) return to_be_explored;
    std::cout<<"didnlt return"<<std::endl;
    ...
    while (iterator != children.end())
    {
        std::cout<<"inside while children"<<std::endl;
        ...
        child->transform_ast();
        std::cout<<"after transform ast"<<std::endl;
        if (!child->ast_is_legal()) {
            if (std::atoi(read_env_var("AS_VERBOSE"))==1){
                // print deleted Ast
                child->print_previous_optims();
            ...
            std::cout<<"before add child path "<<&parent_trace<<std::endl;
             
            parent_trace->add_child_path(child, schedules_annotations->size());
            std::cout<<"after add_child_path "<<std::endl;
            ...
            std::cout<<"push and return "<<std::endl;
            ++iterator;  
    ...
    std::cout<<"add child path"<<std::endl;                
...
void mcts::search_save_matrix(std::vector<syntax_tree*> asts, std::vector<std::string> *schedules_annotations, candidate_trace *parent_trace, float schedule_timeout)
{
    std::cerr<< "mcts::search_save not yet implemented" << std::endl;
    exit(1);
}
/*
std::vector<syntax_tree*> mcts::explore_node(syntax_tree& ast, std::vector<std::string> *schedules_annotations, candidate_trace *parent_trace, float schedule_timeout=0)=0;
{
    std::cerr<< "mcts::search_save not yet implemented" << std::endl;
    exit(1);
}
std::vector<syntax_tree*> mcts::explore_node_classic(syntax_tree& ast, std::vector<std::string> *schedules_annotations, candidate_trace *parent_trace, float schedule_timeout=0)=0;
{
    std::cerr<< "mcts::search_save not yet implemented" << std::endl;
    exit(1);
}
std::vector<syntax_tree*> beam_search_topk::explore_node(syntax_tree& ast, std::vector<std::string> *schedules_annotations, candidate_trace *parent_trace, float schedule_timeout=0)=0;
{
    std::cerr<< "mcts::search_save not yet implemented" << std::endl;
    exit(1);
}
std::vector<syntax_tree*> beam_search_topk::explore_node_classic(syntax_tree& ast, std::vector<std::string> *schedules_annotations, candidate_trace *parent_trace, float schedule_timeout=0)=0;
{
    std::cerr<< "mcts::search_save not yet implemented" << std::endl;
    exit(1);
}
...
void beam_search_topk::search_save_matrix(std::vector<syntax_tree*> asts, std::vector<std::string> *schedules_annotations, candidate_trace *parent_trace, float schedule_timeout)
{
    std::cerr<< "beam_search_topk::search_save not yet implemented" << std::endl;
    exit(1);
}
