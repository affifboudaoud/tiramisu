...
int main(int argc, char **argv)
{
    ...
    // Layer I
    // ------------------------------------------------------- 
    var xi("xi", 0, 98), yi("yi", 0, 34), ci("ci", 0, 5);
    var x("x", 1, 98-1), y("y", 1, 34-1), c("c", 1, 5-1);
    ...
    // -------------------------------------------------------
    //Buffers
    buffer input_buf("input_buf", {5, 34, 98}, p_float64, a_input);
    buffer output_buf("output_buf", {5,34, 98}, p_float64, a_output);
	...
	as.sample_search_space_random_matrix("./function_blur_SMALL_explored_schedules.json", true);
	delete scheds_gen;
	delete model_eval;
	delete bs;
	return 0;
