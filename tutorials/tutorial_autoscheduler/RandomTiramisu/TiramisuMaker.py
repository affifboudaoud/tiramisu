...
class Program:
    ...
    def correct_loop_bounds(self):  # make corrections to loop bounds that are used for accesses with a negative shift
        ...
        for comp in self.ordered_computations_list:
            for read_access in comp.expression.read_access_list:
                # TODO check if the read access is really present in expression after simplification, same for buffer size correction
    ...
    # def delete_unused_buffers(self):  # deletes buffers that are created and not used in the program
    #     used_buffers_list = []
    #     for buffer in self.buffer_list:
    #         for computation in self.ordered_computations_list:
    #             if buffer == computation.write_access.buffer or buffer.wrapping_input.name in computation.expression.expression:
    #                 used_buffers_list.append(buffer)
    #                 break
    #     for buffer in self.buffer_list[:]:
    #         if buffer not in used_buffers_list:
    #             self.buffer_list.remove(buffer)
    #             self.input_list.remove(buffer.wrapping_input)
...
class Computation:
    ...
    def print(self):
        ...
        text += self.name + ': {' + self.write_access.buffer.name + '<--' + ','.join([b.buffer.name for b in self.expression.read_access_list]) + '}'
        print(text)
...
class ReadAccess:
    ...
    def access_pattern_is_stencil(self):  # checks if one or more dimension is accessed using a shifted iterator ( i+ cst)
        if np.any(self.access_pattern[:, -1]):
            return True
        return False
...
if __name__ == '__main__':
    generate_programs(output_path='./output/', first_seed=752605, nb_programs=1)
